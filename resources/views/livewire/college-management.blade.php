        ...
        <div class="border-b border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-950/70 sm:p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                @elseif(auth()->user()->college_id)
                    <flux:button variant="primary" icon="pencil-square" :href="route('colleges.edit', auth()->user()->college_id)" wire:navigate>আমার কলেজ প্রোফাইল সম্পাদনা</flux:button>
                @endif
            </div>
                ...
                <flux:table.rows>
                    @forelse($colleges as $college)
                        <flux:table.row wire:key="college-{{ $college->id }}">
                            ...
                            <flux:table.cell><div class="flex flex-wrap justify-end gap-2"><flux:button size="sm" icon="eye" :href="route('colleges.show', $college)" wire:navigate>দেখুন</flux:button><flux:button size="sm" icon="pencil-square" :href="route('colleges.edit', $college)" wire:navigate>সম্পাদনা</flux:button>@if(auth()->user()->isAdmin() && $college->approval_status === \App\Enums\ApprovalStatus::Pending)<flux:button size="sm" variant="primary" wire:click="approveCollege({{ $college->id }})">এপ্রুভ</flux:button>@endif @if(auth()->user()->isAdmin())<flux:button size="sm" variant="danger" wire:click="delete({{ $college->id }})" wire:confirm="কলেজটি মুছবেন?">মুছুন</flux:button>@endif</div></flux:table.cell>
                        </flux:table.row>
                    @empty
