<div class="space-y-8">
    ...
    <section class="sticky top-20 z-20 rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-lg shadow-slate-200/50 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 dark:shadow-black/20 sm:p-5">
        <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_280px_auto]">
            <div class="relative">
                ...
                <input id="college-search" type="search" wire:model.live.debounce.350ms="search"
                       placeholder="কলেজের নাম, কোড অথবা বিষয় লিখুন"
            ...
            <div>
                <label for="division-filter" class="sr-only">বিভাগ নির্বাচন করুন</label>
                <select id="division-filter" wire:model.live="division"
                    ...
                    @foreach($divisions as $divisionOption)
                        <option value="{{ $divisionOption->id }}">{{ $divisionOption->bn_name ?: $divisionOption->name }}</option>
                    @endforeach
                </select>
            </div>
            @if($search !== '' || $division !== '')
                <button type="button" wire:click="clearFilters" class="rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-600 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-red-500/10">ফিল্টার মুছুন</button>
            @endif
        </div>
        <div wire:loading.flex wire:target="search,division,clearFilters" class="mt-3 items-center gap-2 text-xs font-semibold text-emerald-700 dark:text-emerald-400">
    ...
    <div wire:loading.class="opacity-50" wire:target="search,division,clearFilters" class="grid gap-5 transition-opacity sm:grid-cols-2 xl:grid-cols-3">
