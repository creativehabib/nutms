<div class="space-y-8">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <dl class="divide-y divide-slate-100 dark:divide-slate-800">
                @foreach([
                    ...
                    'শিক্ষার্থীভিত্তিক ধরন' => ['B' => 'বয়েজ এন্ড গার্লস মিশ্র কলেজ', 'F' => 'শুধু গার্লস কলেজ'][$college->male_female] ?? 'উল্লেখ নেই',
                    'মোট জমি' => $college->total_land ?: 'উল্লেখ নেই',
                    'ফোন' => $college->college_phone ?: 'উল্লেখ নেই',
                    'ইমেইল' => $college->college_email ?: 'উল্লেখ নেই',
                    'ওয়েবসাইট' => $college->college_website ?: 'উল্লেখ নেই',
                ] as $label => $value)
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="border-b border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-800/50">
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">অধিভুক্ত বিষয় ও কোর্সসমূহ</h2>
        </div>
        <div class="grid gap-5 p-6 md:grid-cols-2">
            ...
            @empty
                <p class="text-slate-500 md:col-span-2">এই কলেজে এখনো কোনো অধিভুক্ত বিষয় বা কোর্স যোগ করা হয়নি।</p>
            @endforelse
        </div>
    </section>
            ...
            <div class="flex items-end justify-between gap-4">
                ...
                @if($relatedColleges->count() > 1)
                    <div class="flex shrink-0 gap-2">
                        <flux:button type="button" variant="outline" size="sm" icon="chevron-left" data-carousel-previous aria-label="{{ __('Previous colleges') }}" />
                        <flux:button type="button" variant="outline" size="sm" icon="chevron-right" data-carousel-next aria-label="{{ __('Next colleges') }}" />
                    </div>
                @endif
            </div>

            <div class="overflow-hidden" data-carousel-viewport>
                <div class="-mx-2 flex pb-2">
                    @foreach($relatedColleges as $relatedCollege)
                        <div wire:key="related-college-{{ $relatedCollege->id }}" class="min-w-0 flex-[0_0_100%] px-2 sm:flex-[0_0_50%] lg:flex-[0_0_25%]">
                            <a
