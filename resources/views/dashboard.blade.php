    ...
    <div class="flex w-full flex-1 flex-col gap-6">
        ...
        @elseif(auth()->user()->hasRole('principal'))
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <flux:card><flux:heading size="lg">{{ __('Teachers by Subject') }}</flux:heading><div class="mt-4 grid gap-3">@forelse($principalStats['subjects'] as $subject)<div class="flex items-center justify-between gap-4 rounded-lg bg-zinc-50 px-4 py-3 dark:bg-zinc-800"><span>{{ $subject->name }}</span><flux:badge color="indigo">{{ $subject->teachers_count }} teachers</flux:badge></div>@empty<flux:text>{{ __('No subject statistics are available yet.') }}</flux:text>@endforelse</div></flux:card>
            ...
            @if($teacherStats)
                ...
                <div class="grid gap-5 lg:grid-cols-3">
                    ...
                    <flux:card class="flex flex-col">
                        ...
                        <div class="mt-4 grid gap-3 text-sm"><div class="flex justify-between gap-3"><span class="text-zinc-500 dark:text-zinc-400">{{ __('Subject') }}</span><span class="font-medium text-zinc-950 dark:text-white">{{ $teacherStats['profile']->getRelation('subject')?->name ?: __('Not added') }}</span></div><div class="flex justify-between gap-3"><span class="text-zinc-500 dark:text-zinc-400">{{ __('Designation') }}</span><span class="font-medium text-zinc-950 dark:text-white">{{ $teacherStats['profile']->getRelation('designation')?->name ?: __('Not added') }}</span></div><div class="flex justify-between gap-3"><span class="text-zinc-500 dark:text-zinc-400">{{ __('Retirement Age') }}</span><span class="font-medium text-zinc-950 dark:text-white">{{ $teacherStats['retirementAge'] }} years</span></div></div>
