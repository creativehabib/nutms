...
class AiChatService
{
    ...
    public function reply(AiSetting $setting, array $messages): string
    {
        ...
        try {
            $response = $setting->provider === 'gemini'
                ? $this->requestGemini($setting, $messages)
                : $this->requestOpenAiCompatible($setting, $messages);
     ...
     * @param  array<int, array{role: string, content: string}>  $messages
     */
    private function requestOpenAiCompatible(AiSetting $setting, array $messages): Response
    {
     ...
     * @param  array<int, array{role: string, content: string}>  $messages
     */
    private function requestGemini(AiSetting $setting, array $messages): Response
    {
        ...
        return Http::withHeaders(['x-goog-api-key' => $setting->api_key])
            ...
            ->post($endpoint.'/models/'.rawurlencode($setting->model).':generateContent', [
                'system_instruction' => ['parts' => [['text' => $this->systemPrompt($setting)]]],
                'contents' => $contents,
