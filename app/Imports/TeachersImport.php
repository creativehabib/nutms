...
class TeachersImport implements ToModel, WithStartRow
{
    ...
    // কনস্ট্রাক্টরের মাধ্যমে ফাইলের নাম থেকে পাওয়া কলেজের নাম রিসিভ করা হচ্ছে
    ...
    {
        // ফাইলের ধরন অনুযায়ী কলামের ইনডেক্স ডাইনামিক্যালি বের করার লজিক
        $offset = 0;
        ...
        // ডাইনামিক অফসেট (offset) দিয়ে ডেটা বের করা
        $tmisId = $row[2 + $offset] ?? null;
        $name   = $row[4 + $offset] ?? null;

        if (!$tmisId && !$name) {
            return null; // ফাঁকা রো স্কিপ করা
        }
        ...
        if ($contactInfo) {
            // ইমেইল এবং মোবাইল স্পেস বা লাইন-ব্রেক দিয়ে আলাদা করা
            $parts = preg_split('/[\s\r\n]+/', $contactInfo);
        ...
        if ($tmisId) {
            return Teacher::updateOrCreate(['tmis_id' => $tmisId], $data);
        } else {
